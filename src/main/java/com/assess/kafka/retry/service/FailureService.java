...
public class FailureService {
    ...
    @Value(value = "${spring.kafka.library.topic.create-library}")
    private String topic;

    @Autowired
    public FailureService(FailureRecordRepository failureRecordRepository, KafkaTemplate<String, LibraryEvent> kafkaTemplate,
    ...
    private boolean retryOperation(FailureRecord failureRecord) {
        try {
            LibraryEvent libraryEvent = objectMapper.readValue(failureRecord.getMessage(), LibraryEvent.class);
            kafkaTemplate.send(topic, libraryEvent.getEventType().toString(), libraryEvent);
            return true;
        } catch (Exception e) {
            return false;
        }
